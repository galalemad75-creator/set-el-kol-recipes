(async function(){
  ...
  if(ok){
    ...
    localStorage.setItem=function(k,v){orig(k,v);supa.from(typeof SITE_ID!=='undefined'&&SITE_ID?SITE_ID+'_kv_store':'kv_store').upsert({key:k,value:JSON.parse(v),updated_at:new Date().toISOString()},{onConflict:'key'}).catch(()=>{})};
  }
